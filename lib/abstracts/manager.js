const ManagerView = require('../views/manager-view')
...
module.exports = class NetworksManager {
  ...
  constructor (options) {
     ...
     * @type {View}
     */
    this._view = new ManagerView(this.options, this.options.get('manager-view'))
  }
   ...
   */
  addPhysicalNeighbour (peer, layer, options = {}) {
    throw new Error('not yet implemented')
  }

  /**
   * (called from a network) Add a neighbour to our view.
   * If it does not exist yet, connect, then add
   * @param {string} peer the Peer id to add to our list of neighbours
   * @param {Object} [options={}] options to pass when adding a neighbour
   */
  addNeighbourFromNetwork (peer, options = {}) {
    throw new Error('not yet implemented')
  }
