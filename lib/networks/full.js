...
module.exports = class FullConnected extends AbstractNetwork {
  constructor (name, options) {
     ...
     */
    this.options.events.manager.on('add:physical', async (id) => {
      await this.join(id)
    })
  }
  ...
  async join (id) {
    ...
    if (!view.includes(id)) {
      ...
      try {
        ...
        await this.manager.addNeighbourFromNetwork(id, false, this)
        toAdd.push(id)
  ...
  async receive (id, data, options) {
    ...
    if (data.order === 'connect-to-all') {
      ...
      for (let i = 0; i < data.to.length; i++) {
        const to = data.to[i]
        try {
          await this.manager.addNeighbourFromNetwork(to, false, this)
          toAdd.push(to)
          // await this.manager.bindPeerToNetworkFromNetwork(to, this)
        } catch (e) {
          //
        }
      }
      ...
      if (!view.includes(data.to)) {
        try {
          await this.manager.addNeighbourFromNetwork(data.to, false, this)
          toAdd.push(data.to)
          // await this.manager.bindPeerToNetworkFromNetwork(data.to, this)
        } catch (e) {
          //
        }
