...
module.exports = class Manager extends NetworksManager {
  ...
  /**
   * Connect the peer using the current layer to the specified peer if any.
   * Or initiate the first connection
   * @param  {string|undefined}  [id=undefined] peer identifier or undefined
   * @param  {Object}  [options={}]
   * @return {Promise}
   */
  async join (id = undefined, options = {}) {
    debug('[%s] is joining', this.id)
    if (!id) {
      // first connection, if it returns undefined, it means we are alone otherwise return the peer we will be connected with
      ...
      for (let i = 0; i < networks.length; i++) {
        if (peer) {
          this.addPhysicalNeighbour(peer, networks[i])
        }
        // at the end, for the all network initiate the join after the first connection
  ...
  async connect (network, to = undefined, by = undefined, occ = false) {
    ...
    if (this._view.has(network, to) && occ) {
      this._view.add(network, to)
    } else if (!this._view.has(network, to) && !by) {
      // two possibilities, the layer allows this kind of connections or not
      ...
      if (p) this._view.add(network, p)
    } else if (!this._view.has(network, to) && by) {
      // two possibilities, the layer allows this kind of connections
      // or only allow connections by exchanging offers through peer already connected. Then if the connection is not possible
      const p = await this._layer.connect(to, { by })
      if (p) this._view.add(network, p)
    } else if (this._view.has(network, to) && this._view.has(network, by) && to === by) {
  ...
  async send (id, data, options = {}, reply = false) {
    let peer
    try {
      peer = await this._view.get(id)
    } catch (e) {
      peer = this._layer.has(id)
    }
    if (peer || (reply && !peer)) {
      if (this._layer) {
        data.__options = { ...options }
        if (data.__options.reply) {
          data.__options.reply = v4()
   ...
   * @param {string} peer peer id to add to the global view
  ...
  addPhysicalNeighbour (peer, network = undefined) {
    ...
    if (!this._view.has(network, peer)) {
      this._view.add(network, peer)
    }
  }

  /**
   * Global getter on the view.
   * Be aware it will lock all view until a merge has been called on each network. Use at your own risk.
   * @return {Object} See return of ManagerView.get
   */
  get neighbours () {
    console.warn('Be aware that it locks all view until a merge has been called on each network. Use at your own risk.')
    const arr = []
    this._networks.forEach(net => arr.push({
      network: net,
      view: this._view.get(net)
    }))
    return arr
  }
  ...
  setLayer (layer) {
    if (this._layer) {
      ...
      debug('[%s] adding layer: ', this.id, layer.name)
      this._layer = layer
      ...
      this.options.events.layer.on('add:physical', peer => {
        this.networks.forEach(net => {
          net.addNeighbour(peer).then(() => {
            debug('[%s][setLayer/callback] adding neighbour (%s) to network (%s): ', this.id, peer, net.name)
